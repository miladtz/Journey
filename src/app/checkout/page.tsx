...
export default function CheckoutPage() {
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    const summary = cart.items
      .map((item) => `${locale === "fa" ? item.nameFa : item.nameEn} x${item.quantity}`)
      .join(", ");

    const address = [data.get("address"), data.get("city"), data.get("postalCode"), data.get("country")]
      .filter(Boolean)
      .join(", ");
    ...
    await fetch("/api/leads", {
      ...
      body: JSON.stringify({
        ...
        message: `New order (total $${cart.subtotal.toLocaleString()}): ${summary}. Ship to: ${address}.`,
        source: "checkout",
      }),
    });
    ...
    <div className="mx-auto max-w-3xl px-4 py-16 sm:px-6 sm:py-24 lg:px-8">
      ...
      <div className="mt-12 grid gap-10 lg:grid-cols-5">
        <form onSubmit={handleSubmit} className="space-y-5 lg:col-span-3">
          <FormInput label={fields.name} name="name" required />
          <FormInput label={fields.email} name="email" type="email" required />
          <FormInput label={fields.address} name="address" required />
          <div className="grid grid-cols-2 gap-5">
            <FormInput label={fields.city} name="city" required />
            <FormInput label={fields.postalCode} name="postalCode" required />
          </div>
          <FormInput label={fields.country} name="country" required />
